...
const AllMeasurementQuizQuestion = () => {
    ...
    useEffect(() => {
        ...
        async function getQuiz() {
            setLoading(true)
            console.log('answer to be submitted in live quiz', item)
            const payload = {
                quiz_id: item?.id,
            }
            console.log(payload)
            try {
                const res = await HomePageService.getMeasurementQuizQuestions(
                    payload
                )
                if (res.code === -2) {
                    setLoading(false)
    ...
    async function submitLessonQuiz() {
        ...
        console.log(payload)
        try {
            const res = await HomePageService.submitMeasurementQuizQuestions(
                payload
            )
            if (res.code === 200) {
...
export default AllMeasurementQuizQuestion
