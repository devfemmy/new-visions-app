import { View, Text, TextInput, StyleSheet, KeyboardAvoidingView, ScrollView, FlatList, TouchableOpacity } from 'react-native'
...
export default function Sons({navigation}) {
    ...
    const handleAddSon = () => {
        ...
        if (reg.test(searchMail) === false) {
            Toast.show({
            text1:(I18n.locale === 'ar'?"البريد الالكتروني غير صحيح": "Wrong Mail Format"),
            type:'error',
...
email:searchMail
    ...
    const renderItem = ({item})=>(
        <SonListItem name={item.name}
        status={item.status} 
        subClick={() =>{SubscriptionsClicked(item)}}
        attendanceClick={() =>{AttendanceClicked(item)}}
        />
    );
  return (
    <View style={{backgroundColor:colors.white,  flex:.4}}>

        <View><Text style={styles.title}>أضف ابن جديد</Text></View>
        <View style={{flexDirection:'row', alignItems:'center', justifyContent:'center', borderRadius:15}}>
        ...
        <TouchableOpacity style={{width:'25%'}} onPress={()=>{handleAddSon();}} >
